...
const activityConfig: Record<string, { icon: string; verb: string; color: string }> = {
  shipped: { icon: '\u2713', verb: 'Shipped', color: 'text-green-600 bg-green-50' },
  building: { icon: '\u2699', verb: 'Building', color: 'text-blue-600 bg-blue-50' },
  pr_created: { icon: '\u2192', verb: 'PR created', color: 'text-violet-600 bg-violet-50' },
  idea_proposed: { icon: '+', verb: 'Idea', color: 'text-gray-600 bg-gray-100' },
}
    ...
    return (
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="animate-pulse">
          <div className="h-8 bg-gray-100 rounded-lg w-64 mb-2" />
          <div className="h-5 bg-gray-50 rounded w-48 mb-10" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-44 bg-gray-50 rounded-xl" />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      {isNewUser && (
        <section className="mb-12">
          <div className="rounded-xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-6 sm:p-8">
            <h2 className="text-lg font-semibold text-gray-900 mb-1">Get started</h2>
            <p className="text-sm text-gray-500 mb-6">Three steps to your first contribution.</p>
            <div className="grid sm:grid-cols-3 gap-6">
              <div className="flex gap-3">
                <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-900 text-white text-sm font-bold shrink-0">1</div>
                <div>
                  <h3 className="text-sm font-medium text-gray-900">Pick a project</h3>
              ...
              <div className="flex gap-3">
                <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-900 text-white text-sm font-bold shrink-0">2</div>
                <div>
                  <h3 className="text-sm font-medium text-gray-900">Chat with the agent</h3>
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
              ...
              <div className="flex gap-3">
                <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-900 text-white text-sm font-bold shrink-0">3</div>
                <div>
                  <h3 className="text-sm font-medium text-gray-900">Watch it ship</h3>
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
      ...
      {(hasProjects || hasContributions) && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-12">
          <div className="p-4 rounded-xl border border-gray-200 bg-white">
            <div className="text-2xl font-semibold text-gray-900">{data.stats.total_projects}</div>
            <div className="text-sm text-gray-500 mt-0.5">Projects</div>
          </div>
          <div className="p-4 rounded-xl border border-gray-200 bg-white">
            <div className="text-2xl font-semibold text-gray-900">{data.stats.total_in_progress}</div>
            <div className="text-sm text-gray-500 mt-0.5">In progress</div>
          </div>
          <div className="p-4 rounded-xl border border-gray-200 bg-white">
            <div className="text-2xl font-semibold text-gray-900">{data.stats.total_ideas}</div>
            <div className="text-sm text-gray-500 mt-0.5">Total ideas</div>
          </div>
          <div className="p-4 rounded-xl border border-gray-200 bg-white">
            <div className="text-2xl font-semibold text-gray-900">{data.stats.total_shipped}</div>
            <div className="text-sm text-gray-500 mt-0.5">Shipped</div>
          </div>
        </div>
      )}
      ...
      {isNewUser && hasFeatured && (
        <section className="mb-12">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Projects to explore</h2>
              <p className="text-sm text-gray-500 mt-0.5">Living open source projects you can contribute to right now.</p>
            </div>
            <Link
              href="/explore"
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-3">
              {isPro && (
                <Link
                  href="/new"
                  className="text-sm px-3.5 py-1.5 rounded-lg bg-gray-900 text-white font-medium hover:bg-gray-800 transition-colors"
                >
                  New project
                </Link>
              )}
        ...
        <section>
          ...
          {hasContributions ? (
              ...
              {data.contributions.map((c) => {
                ...
                return (
                  <Link
                    ...
                    className="block p-3 rounded-lg border border-gray-200 hover:border-gray-300 hover:bg-gray-50/50 transition-all"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-gray-900 truncate">{c.title}</div>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs font-mono text-gray-400">{c.project_name}</span>
            ...
            <div className="text-center py-10 border border-dashed border-gray-200 rounded-xl">
              <p className="text-sm text-gray-500 mb-1">No contributions yet</p>
      ...
      {!isPro && (
        <section className="mt-12 rounded-xl border border-gray-200 bg-gray-50 p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="font-semibold text-gray-900">Create your own projects</h3>
            ...
            <Link
              href="/pricing"
              className="px-5 py-2.5 rounded-lg bg-gray-900 text-white text-sm font-medium hover:bg-gray-800 transition-colors shrink-0 text-center"
            >
              View plans
            </Link>
          </div>

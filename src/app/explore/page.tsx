    ...
    return (
      <div className="min-h-screen">
        <div className="max-w-5xl mx-auto px-6 py-16">
          <div className="mb-12">
            <div className="h-9 w-56 bg-canvas-subtle rounded-lg animate-pulse mb-3" />
            <div className="h-5 w-80 bg-canvas-subtle rounded animate-pulse" />
          </div>
    ...
    return (
      <div className="min-h-screen">
        <div className="max-w-5xl mx-auto px-6 py-16 text-center">
          <p className="text-ink-secondary">Failed to load projects</p>
          ...
          <button
            ...
            className="mt-4 text-sm px-4 py-2 rounded-lg border border-line text-ink-secondary hover:border-line hover:text-ink transition-colors"
          >
            Try again
          </button>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="mb-10 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          {isPro && (
            <Link
              href="/new"
              className="px-4 py-2 rounded-lg bg-ink text-canvas text-sm font-medium hover:bg-ink/90 transition-colors"
            >
              New Project
            </Link>
          )}
        ...
        {projects.length > 0 && (
          ...
          <div className="relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-tertiary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              className="pl-9 pr-3 py-1.5 text-sm border border-line rounded-lg bg-surface text-ink placeholder-ink-tertiary focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent w-full sm:w-56"
            />
          </div>
          <div className="flex items-center gap-1 p-1 bg-canvas-subtle rounded-lg w-fit" role="group" aria-label="Filter projects by activity">
            <button
              onMouseEnter={() => filter !== 'all' && setFilter('all')}
            ...
            <button
              onMouseEnter={() => filter !== 'active' && setFilter('active')}
            ...
            <button
              onMouseEnter={() => filter !== 'idle' && setFilter('idle')}
            ...
            >
              <span className="w-1.5 h-1.5 rounded-full bg-ink-tertiary" aria-hidden="true" />
              Idle ({idleCount})
            </button>
          </div>
          <select
            ...
            className="px-3 py-1.5 text-sm border border-line rounded-lg bg-surface text-ink-secondary focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent sm:ml-auto"
          >
        ...
        {filteredProjects.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-line rounded-xl">
            {projects.length === 0 ? (
              <>
                <p className="text-ink-secondary">No public projects yet.</p>
              ...
              <>
                <p className="text-ink-secondary">No projects match &ldquo;{search}&rdquo;</p>
                <button
                  onClick={() => setSearch('')}
                  className="text-sm text-ink-tertiary hover:text-ink-secondary mt-2 underline underline-offset-2"
                >
                  Clear search
                </button>
              </>
              ...
              <>
                <p className="text-ink-secondary">No {filter === 'active' ? 'active' : 'idle'} projects{search.trim() ? ` matching "${search}"` : ''}</p>
                <button
                  onClick={() => setFilter('all')}
                  className="text-sm text-ink-tertiary hover:text-ink-secondary mt-2 underline underline-offset-2"
                >
